  ...
  describe('Reset password page', () => {
    ...
    it('Verify that user unable to trigger email for not registered user', () => {
      cy.fixture('auth').then(data => {
        cy.get(`${data.path.forgotPassword.submitBtn} button`)
          .should('be.disabled');
        cy.get(data.path.forgotPassword.emailField)
          .type('notValidEmailaddress@coo.cc');
        cy.get(`${data.path.forgotPassword.submitBtn} button`)
          .should('not.be.disabled');
        cy.get(data.path.forgotPassword.submitBtn)
          .click();
  ...
  describe('Login page', () => {
    ...
    it('Verify validation for email field', () => {
      cy.fixture('auth').then((auth) => {
        ...
        cy.get(`${auth.path.login.submitBtn} button`)
          .should('be.disabled');
    ...
    it('Verify validation for password field', () => {
      cy.fixture('auth').then((auth) => {
        ...
        cy.get(`${auth.path.login.submitBtn} button`)
          .should('be.disabled');
        cy.get(auth.path.login.passwordField)
          .contains('Please fill the field')
          .type('test');
        ...
        cy.get(`${auth.path.login.submitBtn} button`)
          .should('be.disabled');
        cy.get(auth.path.login.passwordField)
          .contains('Field should be at least 6 chars length');
      });
    });
  ...
  describe('Registration page', () => {
    ...
    it('Verify validation for name and age fields', function () {
      cy.fixture('auth').then((auth) => {
        ...
        cy.get(auth.path.reg.nameField)
          .click()
          .contains('Please fill the field');
        cy.get(auth.path.reg.ageField)
          .contains('Please fill the field');
        cy.get(`${auth.path.reg.submitBtn} button`)
          .should('be.disabled');
      });
    });
    ...
    it('Verify validation for email field', () => {
      cy.fixture('auth').then((auth) => {
        ...
        cy.get(`${auth.path.reg.submitBtn} button`)
          .should('be.disabled');
        cy.get(auth.path.reg.emailField)
          .contains('Please fill the field');
      });
    });
    ...
    it('Verify that user able to sign up with already registered email', () => {
      cy.fixture('auth').then((auth) => {
        cy.get(`${auth.path.reg.nameField} input`)
          .type(`Test ${testIdx}`);
        cy.get(`${auth.path.reg.ageField} input`)
          .type(testIdx.toString());
        cy.get(`${auth.path.reg.emailField} input`)
          .type(auth.email);
        cy.get(`${auth.path.reg.passwordField} input`)
          .type(password);
        cy.get(`${auth.path.reg.confirmPasswordField} input`)
          .type(password);
        cy.get(auth.path.reg.submitBtn)
          .click();
    ...
    it('Verify that user able to sign up with proper user data ', () => {
      cy.fixture('auth').then((auth) => {
        ...
        cy.get(`${auth.path.reg.ageField} input`)
          .type(testIdx.toString());
        cy.get(`${auth.path.reg.emailField} input`)
          .type(`nazaritikus+${testIdx}@gmail.com`);
        cy.get(`${auth.path.reg.passwordField} input`)
          .type(password);
        cy.get(`${auth.path.reg.confirmPasswordField} input`)
          .type(password);
